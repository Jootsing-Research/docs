...
Phone-specific REST paths require `phone_id`, even if your account has only one connected phone. Call `GET /phones` to find the ID before sending a command.
...
| `PATCH /phones/{id}/settings` | Update the phone's display name |
