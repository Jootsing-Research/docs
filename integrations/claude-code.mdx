...
Claude Code asks for approval before an MCP tool's first use in the default permission mode. When prompted, you can allow the TapKit tools for future calls, or run `/permissions` and add `mcp__tapkit__*` to your allow rules. Leave the default prompts in place if you want to confirm phone actions individually.
